...
// ── Precompute frame-level targets ────────────────────────────────────────────
...
static void build_frame_targets(RecordingFrames& rf, int n_pitches) {
    ...
    for (const auto& note : rf.notes) {
        ...
        // Soft onset target: Gaussian centered on onset frame
        // This gives the network a gradient even if slightly off
        float sigma = 1.5f;  // ~1.5 frames width
        ...
        // Frame target: 1 for entire duration
